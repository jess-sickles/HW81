...
def get_tweets():
    if 'umsi' in CACHE_DICTION:
        print("Using cache")
        s = CACHE_DICTION['umsi']
    else:
        print("Fetching from internet")
        s = api.user_timeline('umsi')
        #adds to cache
...
cursor.execute('CREATE TABLE Tweets (tweet_id TEXT, author TEXT, time_posted TIMESTAMP, tweet_text TEXT, retweets NUMBER)')
